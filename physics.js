...
function applyExpression(emotion) {
  // Delegate to extended version if available (registered by interactions.js)
  if (typeof window._applyExpressionExtended === 'function') {
    return window._applyExpressionExtended(emotion);
  }
  _applyExpressionBase(emotion);
}

function _applyExpressionBase(emotion) {
